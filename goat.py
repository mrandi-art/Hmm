...
YOUR_USERNAME = "Lost_znx"                                # OWNER USERNAME (without @)
UPDATE_CHANNEL = "sukmynutsqt"                             # CHANNEL USERNAME (without @)
...
# --- Security Functions ---
def ai_security_scan(file_content, file_name):
    """Uses AI to read the code and summarize what it does."""
    if not GEMINI_API_KEY:
        return "⚠️ AI Scanner disabled (No API Key). Please review this file manually."
    try:
        prompt = f"""
        You are a senior cybersecurity analyst. A user just uploaded this file ({file_name}).
        Read the code and write a short, concise summary of exactly what it does.
        State clearly if it appears SAFE or DANGEROUS, and explicitly list any suspicious behaviors 
        (e.g., reverse shells, file deletion, token stealing, crypto mining, bypassing scanners).
        Keep the response highly informative but under 500 characters.
        ...
        return f"🤖 AI Scan failed: {str(e)}"

def check_code_security(file_path, file_type):
    """Check code using AI and force Admin Approval"""
    try:
        ...
        ai_analysis = ai_security_scan(content, file_path)
        
        # ALWAYS return False to force Admin Approval, passing the AI Analysis as the message
        return False, ai_analysis
    except Exception as e:
        logger.error(f"Error in security check: {e}")
        return False, f"Security check error: {str(e)}"

def scan_zip_security(zip_path):
    """Check ZIP contents and force Admin Approval"""
    # ALWAYS force manual approval for ZIP archives
    return False, "🤖 AI cannot deeply scan all files inside a ZIP. Manual admin review and approval is required."
...
def kill_process_tree(process_info):
    ...
    try:
        ...
        # Rootless Docker Container Cleanup
        process = process_info.get('process')
        if process and hasattr(process, 'pid'):
            try:
                if sys.platform.startswith('linux'):
                    # Kills the entire container process group instantly (No zombies)
...
def handle_zip_file(downloaded_file_content, file_name_zip, message):
    ...
    try:
        ...
        # Security check for ZIP (always forces approval now)
        is_safe, security_msg = scan_zip_security(zip_path)
        if not is_safe:
            # Send approval request to admin
            security_warning_msg = f"🚨 Archive needs approval:\n👤 User: {user_id}\n📁 File: {file_name_zip}\n\n⚠️ **Details:**\n{security_msg}"
            markup = types.InlineKeyboardMarkup()
            markup.row(
...
def run_script(script_path, script_owner_id, user_folder, file_name, message_obj_for_reply, attempt=1):
    """Run Python script with Absolute Paths and Auto-Dependency Fix."""
    max_attempts = 3
    if attempt > max_attempts:
        bot.reply_to(message_obj_for_reply, f"❌ Failed to start '{file_name}' after {max_attempts} attempts. Check logs.")
        return

    script_key = f"{script_owner_id}_{file_name}"
    
    # 1. Generate ABSOLUTE paths (Fixes most 'not found' issues)
    abs_user_folder = os.path.abspath(user_folder)
    abs_modules_path = os.path.join(abs_user_folder, 'modules')
    os.makedirs(abs_modules_path, exist_ok=True)
    
    # 2. Force Environment Variables
    env = os.environ.copy()
    env["PYTHONPATH"] = abs_modules_path + os.pathsep + env.get("PYTHONPATH", "")
    env["PYTHONUNBUFFERED"] = "1"

    logger.info(f"Attempt {attempt}: Starting {file_name} in {abs_user_folder}")

    try:
        # --- PHASE 1: IMPORT VALIDATION ---
        check_command = [sys.executable, "-u", script_path]
        ...
        try:
            # Wait to see if it crashes immediately due to missing modules
            stdout, stderr = check_proc.communicate(timeout=4)
        except subprocess.TimeoutExpired:
            # No crash in 4 seconds = Imports are likely perfect!
            check_proc.kill()
            stderr = "" 

        if check_proc.returncode != 0 and stderr:
            # Improved Regex: Detects base package from errors like 'telebot.types' or 'telethon.sync'
            match = re.search(r"(?:ModuleNotFoundError: No module named|ImportError: No module named) '(.+?)'", stderr)
            if match:
                full_module_name = match.group(1).strip().strip("'\"")
                # Fix: If error is 'telethon.sync', we install 'telethon'
                ...
                if success:
                    ...
                    return run_script(script_path, script_owner_id, abs_user_folder, file_name, message_obj_for_reply, attempt + 1)
                else:
                    bot.reply_to(message_obj_for_reply, f"❌ Auto-install failed for `{base_module}`.")
                    return
            else:
                 # It crashed for a reason other than a missing module
                 bot.reply_to(message_obj_for_reply, f"❌ Script Error:\n```\n{stderr[:600]}\n```", parse_mode='Markdown')
                 return

        # --- PHASE 2: LONG RUN EXECUTION ---
        log_file_path = os.path.join(abs_user_folder, f"{os.path.splitext(file_name)[0]}.log")
        log_file = open(log_file_path, 'w', encoding='utf-8', errors='ignore')
        
        cmd = [sys.executable, "-u", file_name]
        if sys.platform.startswith('linux'):
            try: cmd = ['prlimit', '--as=1073741824'] + cmd
            except: pass
        ...
        logger.error(f"Fatal error in run_script: {e}", exc_info=True)
...
def run_js_script(script_path, script_owner_id, user_folder, file_name, message_obj_for_reply, attempt=1):
    ...
    try:
        ...
        try:
            ...
            container_name = f"docker_sandbox_js_{script_owner_id}_{safe_name}"
            
            # Rootless Docker Execution Bridge for JS
            docker_command = ['node', file_name]
            
            # Container Memory Limit Simulation (1GB RAM limit)
            if sys.platform.startswith('linux'):
            ...
            logger.info(f"Started Rootless Docker JS process {process.pid} (Container: {container_name}) for {script_key}")
            bot_scripts[script_key] = {
                'process': process, 'log_file': log_file, 'file_name': file_name,
...
def check_dkr_status(message):
    ...
    # Stackhost bypass validation
    bot.reply_to(message, "✅ **Rootless Docker Engine Active:**\nContainerization bridge is operating in user-space (Stackhost bypass mode enabled). All containers isolated via `setsid` and `prlimit`.\n\n🤖 **Gemini AI Scanner:** " + ("ACTIVATED" if GEMINI_API_KEY else "DISABLED"), parse_mode='Markdown')
...
def handle_file_upload_doc(message):
    ...
    try:
        ...
        if file_ext == '.zip':
            ...
            # Security check for script files (always forces approval now)
            is_safe, security_msg = check_code_security(file_path, file_ext[1:])
            if not is_safe:
                # Send AI analysis to admin for approval
